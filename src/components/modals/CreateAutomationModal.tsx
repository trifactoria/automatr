    ...
    <Modal open={open} title="Create Automation" onClose={onClose}>
      <div className="grid gap-3">
        ...
        <label className="grid gap-1">
          ...
          <input className="rounded-lg border p-2" value={desc} onChange={(e) => setDesc(e.target.value)} />
        </label>
        <label className="grid gap-1">
          <span className="text-sm text-gray-600">YAML</span>
          <textarea className="h-64 rounded-lg border p-2 font-mono text-sm" value={yaml} onChange={(e) => setYaml(e.target.value)} />
        </label>
